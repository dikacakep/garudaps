...
export default function LeaderboardSection() {
  ...
  return (
    <section id="leaderboard" className="relative py-20 md:py-32 bg-[#0a0a0a] overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-200 h-200 bg-orange-600/5 blur-[120px] rounded-full pointer-events-none transform-gpu" />
            ...
            >
              {restPlayers.map((player) => (
                <motion.div
                  ...
                  className="group relative flex items-center justify-between p-3 md:p-5 rounded-xl bg-white/5 border border-white/5 hover:border-orange-500/30 backdrop-blur-sm transition-colors duration-300 will-change-transform"
                >
...
function PodiumCard({ player, rank, color, height, isFirst = false, delay }: PodiumCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`relative flex-1 ${height} flex flex-col items-center justify-end pb-0 group min-w-0 transform-gpu`}
    >
      {/* Crown Rank 1 */}
      {isFirst && (
        <>
      ...
      <div className="text-center mb-2 md:mb-6 px-0.5 w-full relative z-20 flex flex-col items-center">
        <div
          className={`mb-1 md:mb-3 w-5 h-5 md:w-14 md:h-14 rounded-full flex items-center justify-center text-black font-black text-[9px] md:text-2xl border md:border-4 border-[#0a0a0a] shadow-xl ${
        ...
        <h3
          className={`font-black text-[9px] md:text-2xl truncate drop-shadow-md mb-0.5 max-w-full w-full tracking-tight px-1 ${
        ...
        <div
          className={`inline-flex items-center gap-1 md:gap-2 px-1.5 py-0.5 md:px-4 md:py-1 rounded-full border shadow-lg ${
            isFirst
        ...
        >
          ...
          />
          <span
            className={`font-bold font-mono text-[8px] md:text-sm uppercase ${
      ...
      {/* Podium Pillar */}
      <div
        className={`w-full flex-1 rounded-t-[20px] md:rounded-t-[30px] bg-linear-to-b ${color} relative overflow-hidden shadow-2xl`}
      >
